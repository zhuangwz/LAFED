...
class DVERGE_Trainer():
    ...
    def train(self, epoch):
        ...
        for batch_idx, (si, sl, ti, tl) in enumerate(batch_iter):
            ...
            image_path = "results/distill_examples/"
            # Image.fromarray(np.uint8(si.cpu().numpy() * 255.0)).convert('RGB').save(image_path + str(ps) + "_source.png", "png")
            # Image.fromarray(np.uint8(ti.cpu().numpy() * 255.0)).convert('RGB').save(image_path + str(ps) + "_target.png", "png")

            s_img = si[0].clone().detach() * 255.0
            s_img = s_img.permute(1, 2, 0).to('cpu', torch.uint8).numpy()
            cv2.imwrite(image_path + str(ps) + "_label" + str(sl.to(torch.device('cpu')).numpy()[0]) + "_source.jpeg", s_img, [int(cv2.IMWRITE_JPEG_QUALITY),95])
            # im = Image.fromarray(s_img, mode="RGB")
            # im.save(image_path + str(ps) + "_label" + str(sl.to(torch.device('cpu')).numpy()[0]) + "_source.png")
            # vutils.save_image(s_img[:1], image_path + str(ps) + "_label" + str(sl.to(torch.device('cpu')).numpy()[0]) + "_source.png", padding=0)
            t_img = ti[0].clone().detach() * 255.0
            t_img = t_img.permute(1, 2, 0).to('cpu', torch.uint8).numpy()
            im = Image.fromarray(t_img, mode="RGB")
            im.save(image_path + str(ps) + "_label" + str(tl.to(torch.device('cpu')).numpy()[0]) + "_target.png")
            # vutils.save_image(t_img[:1], image_path + str(ps) + "_label" + str(tl.to(torch.device('cpu')).numpy()[0]) + "_target.png", padding=0)
            for i in range(len(distilled_data_list)):
                # Image.fromarray(np.uint8(img.numpy() * 255.0)).convert('RGB').save(image_path + str(ps) + "_" + str(i) + ".png", "png")
                img = distilled_data_list[i]
                t_img = img.clone().detach()
                t_img = t_img.to(torch.device('cpu'))
                vutils.save_image(t_img[:1], image_path + str(ps) + "_" + str(i) + ".png", padding=0)
...
def main():
    ...
    if args.start_from == 'baseline':
        args.model_file = os.path.join('checkpoints', 'baseline', 'seed_0',
