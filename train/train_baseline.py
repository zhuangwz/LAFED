...
def main():
    ...
    save_root = os.path.join('checkpoints', 'baseline')
    if args.num_class == 100:
        save_root += '_cifar100'
    save_root = os.path.join(save_root, 'seed_{:d}'.format(args.seed), '{:d}_{:s}{:d}'.format(args.model_num, args.arch, args.depth))
