...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # m1,m2,m3,m4 = self.norm()
        if self.dropout_layer == 1:
            x = self.mixup_alpha * x + (1 - self.mixup_alpha) * self.normalizer(self.mixup_feature)
            # x = m1 * self.normalizer(self.mf1[0]) + m2 * self.normalizer(self.mf1[1]) + \
            #     m3 * self.normalizer(self.mf1[2]) + m4 * self.normalizer(self.mf1[3])
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)  # 16x32x32
        if 1 < self.dropout_layer <= 7:
            x = self.mixup_alpha * x + (1 - self.mixup_alpha) * self.normalizer(self.mixup_feature)
            # x = m1 * self.normalizer(self.mf1[0]) + m2 * self.normalizer(self.mf1[1]) + \
            #     m3 * self.normalizer(self.mf1[2]) + m4 * self.normalizer(self.mf1[3])
        x = self.layer1(x)  # 16x32x32
        if 7 < self.dropout_layer <= 13:
            x = self.mixup_alpha * x + (1 - self.mixup_alpha) * self.normalizer(self.mixup_feature)
            # x = m1 * self.normalizer(self.mf1[0]) + m2 * self.normalizer(self.mf1[1]) + \
            #     m3 * self.normalizer(self.mf1[2]) + m4 * self.normalizer(self.mf1[3])
        x = self.layer2(x)  # 32x16x16
        if 13 < self.dropout_layer <= 19:
            x = self.mixup_alpha * x + (1 - self.mixup_alpha) * self.normalizer(self.mixup_feature)
            # x = m1 * self.normalizer(self.mf1[0]) + m2 * self.normalizer(self.mf1[1]) + \
            #     m3 * self.normalizer(self.mf1[2]) + m4 * self.normalizer(self.mf1[3])
        x = self.layer3(x)  # 64x8x8
        if self.dropout_layer == 20:
            x = self.mixup_alpha * x + (1 - self.mixup_alpha) * self.normalizer(self.mixup_feature)
            # x = m1 * self.normalizer(self.mf1[0]) + m2 * self.normalizer(self.mf1[1]) + \
            #     m3 * self.normalizer(self.mf1[2]) + m4 * self.normalizer(self.mf1[3])
        x = self.avgpool(x)
